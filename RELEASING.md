...
2. Update the version number across the project using the `bumpver` tool. See [this section](#choosing-the-next-version-number) for more details about choosing the correct version number.
   ...
   Then, run `just bump` to update the version number, with the appropriate command line arguments -- they will be passed through to the `bumpver` CLI command. See the [`bumpver` documentation](https://github.com/mbarkhau/bumpver) for more details.
   ...
   **Note**: For any of the following commands, you can add the command line flag `--dry` to preview the changes without actually making the changes.
   ...
   ```shell
   just bump --patch  # for a patch release
   just bump --minor  # for a minor release
   just bump --major  # for a major release
   ```
...
6. Draft a [new release](https://github.com/westerveltco/setup-ci-action/releases/new) on GitHub.
